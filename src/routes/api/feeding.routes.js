...
router.get("/feeding_events", (req, res) => {
  FeedingEvent.find({ userEmail: req.user.email }).then((events) => {
    return res.json(events);
  });
});

// @route GET /api/feeding/feeding_events/last
// @desc Gets the latest feeding event
// @access Public
router.get("/feeding_events/last", (req, res) => {
  FeedingEvent.findOne({ userEmail: req.user.email })
    .sort({ fedOn: -1 })
    .then((event) => {
      console.log(
        `The last feeding was ${
          event.amount
        } ounces at ${event.fedOn.getHours()} ${event.fedOn.getMinutes()}`
      );
